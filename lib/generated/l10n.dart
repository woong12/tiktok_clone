...
class S {
  ...
  String signUpTitle(String nameOfTheApp, DateTime when) {
    final DateFormat whenDateFormat =
        DateFormat('LLL dd', Intl.getCurrentLocale());
    final String whenString = whenDateFormat.format(when);

    return Intl.message(
      'Sign up for $nameOfTheApp $whenString',
      name: 'signUpTitle',
      desc: 'The title people see when they open the app for the forst time',
  ...
  /// `Log in {gender, select, all{ } male{sir} female{madam} other{human}}`
  ...
  /// `{value}`
  String bookmarkCount(int value) {
    final NumberFormat valueNumberFormat = NumberFormat.compact(
      locale: Intl.getCurrentLocale(),
    );
    final String valueString = valueNumberFormat.format(value);
